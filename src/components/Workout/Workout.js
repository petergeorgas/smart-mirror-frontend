...
function Workout() {
	...
	const [text, setText] = React.useState(
		"Workout " +
			(imageNum + 1) +
			"/" +
			Math.round(current_workout.length / 2) +
			" " +
			current_workout_names[imageNum]
	);
	const [image, setImage] = React.useState(current_workout[0]);

	//const [resting, setResting] = useState(false);
	const [workoutOver, setWorkoutOver] = useState(false);
	...
	useEffect(() => {
		...
		const interval = setInterval(() => {
			...
			if (!workoutOver) {
				if (!resting) setDuration(prettyMilliseconds(current_time));
