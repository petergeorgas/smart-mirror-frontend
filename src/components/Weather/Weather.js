...
function Weather() {
  ...
  function parseTime(str) { // Parses the time string passed by the API
    const split = str.split("T");
    const splitTimeValue = split[1].split(":");
    return +splitTimeValue[0];
  }

  function checkDay(num) { // Returns true if it is daytime
    if (num > 7 && num < 19) {
      return true;
    }
    return false;
  ...
  useEffect(() => {
    fetch(url)
      ...
      .then(function (data) {
        setTemperature(Math.round(data.current_weather.temperature) + '\u00b0' + 'F');
        const code = data.current_weather.weathercode;

        if (code === 0 || code === 1)
          checkDay(parseTime(data.current_weather.time)) ? setImage(clearSkyDay) : setImage(clearSkyNight);
        else if (code === 2)
          checkDay(parseTime(data.current_weather.time)) ? setImage(partlyCloudyDay) : setImage(parytlyCloudyNight);
        else if (code === 3)
          setImage(cloudy);
        else if (code === 45 || code === 48)
          setImage(fog);
        else if (code === 51 || code === 53 || code === 55 || code === 56 || code === 57 || code === 61 || code === 63 || code === 65 || code === 80 || code === 81 || code === 82)
          setImage(rainy);
